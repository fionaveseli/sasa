...
export function SuccessCard({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (userStr) {
        ...
        // Use fullName if available, otherwise fallback to "User"
        setUserName(userData.fullName || "User");
        if (userData.university_name) {
          setUniversityName(userData.university_name);
        }
      }
    } catch (error) {
  ...
  const handleContinue = () => {
    // Make sure we have a token before redirecting to dashboard
    const token = localStorage.getItem("token");
    if (!token) {
      console.error("No token found, redirecting to login");
      router.push("/login");
    ...
    <Card className="w-full max-w-md p-8" {...props}>
      <CardHeader className="space-y-1 text-center">
        <div className="flex items-center justify-center gap-4">
          ...
          <h1 className="text-3xl tracking-tight">Welcome, {userName}!</h1>
        </div>
      </CardHeader>
      <CardContent className="text-center">
        <p className="font-extralight text-m">
          Your account has been created successfully. You&apos;re all ready to
          proceed using <span className="font-medium text-primary">sasa</span>.
        </p>
        {universityName && (
